...
# Create the continuous target variable from the "Temperature" column
continuous_y = aligned_md['Temperature'].copy()

# Drop rows with NaN in the target and corresponding rows in clr_df
valid_indices = continuous_y.dropna().index
continuous_y = continuous_y.loc[valid_indices]
clr_df = clr_df.loc[valid_indices]

# Bin the Temperature into a three-class target variable:
# 0 if Temperature < 10, 1 if 10 <= Temperature <= 22, else 2.
...
print("Starting Nested Cross-Validation for 'temperature'...")
