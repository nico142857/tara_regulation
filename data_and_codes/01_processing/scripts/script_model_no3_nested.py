...
# Create the continuous target variable from metadata
...
# Bin the NO3 values into a binary target variable: 0 if NO3 < 7, else 1
y_total = continuous_y.apply(lambda x: 0 if x < 7 else 1)
...
def tune_hyperparameters(X_train, y_train, max_evals=100):
    ...
    def objective(params):
        ...
        score = cross_val_score(clf, X_train, y_train, scoring='f1', cv=inner_cv).mean()
        return -score  # Negate because fmin minimizes
...
print("Starting Nested Cross-Validation for NO3...")
for train_idx, test_idx in outer_cv.split(X, y):
    ...
    # Tune hyperparameters on this training fold using inner CV
